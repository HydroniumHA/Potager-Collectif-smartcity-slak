...
const CreateGardenPage = () => {
    ...
    const [currentZoneSelected, setCurrentZoneSelected] = useState(-1);
    ...
    useEffect(()=> {
        //TODO: (Logan) Check si le client a déjà un potager si oui le rediriger vers la page edit du potager 
    ...
    return (
        <>
                ...
                <ScrollView className='flex flex-row flex-wrap px-0 mt-3'>
                    ...
                    <section className='flex flex-row flex-wrap w-full mt-20 px-4 justify-between'>   
                        ...
                        <Container className='w-1/2 aspect-video focus-within:border-4 focus-within:border-green1 '>
                            <PlanViewer editable={true} contentState={[content, setContent]} xCount={xCount} yCount={yCount} saveButtonRef={createButtonRef} onSaveHandler={onCreate} currentZoneSelected={currentZoneSelected}/>
                        </Container>
                        ...
                        <section className='w-2/5 h-[300px]'>
                            <div className={twMerge('transition-opacity opacity-100 ease-in duration-400 w-full h-[350px] flex flex-col space-y-6', animationZoneControllerClassName)}>
                            {
                                zones.slice((zoneCurrentPage - 1) * 5, ((zoneCurrentPage - 1) * 5) + 5).map((zone) => 
                                    <ZoneCapsule key={zone.index} zone={zone} currentZoneState={[currentZoneSelected, setCurrentZoneSelected]}/>
                                )
                            }
                            </div>
            ...
            <button ref={createButtonRef} className='z-20 absolute bottom-8 right-8 bg-white rounded-5xl w-16 h-16 max-2xl:bg-green1'>Create</button>
